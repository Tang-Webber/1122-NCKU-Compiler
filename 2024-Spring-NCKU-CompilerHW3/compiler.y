...
%type <object_ptr> ShiftExpr OtherExpr UnaryExpr ConvertExpr CallFuncExpr ArrayExpr
%type <object_ptr> ForRangeStmt Condition
...
/* Function */
FunctionDefStmt
    : VARIABLE_T IDENT '(' { createFunction($1, $<s_var>2); } FunctionParameterStmtList ')' { funcSigAssign(); } '{' StmtList '}' { dumpScope(); functionEnd($1); }
;
...
SimpleStmt
    ...
    | IDENT '[' { loadArray($<s_var>1);} ArrayInit
...
ArrayInit
    : Expression ']' { loadArrayVariable("test",  $<object_ptr>1); } VAL_ASSIGN Expression ';' { assignArrayVariable("EQL", $<object_ptr>5);}
    | Expression ']' '[' { codeRaw("aaload"); } Expression ']' { load2DArrayVariable("test",  $<object_ptr>1, $<object_ptr>5); } VAL_ASSIGN Expression ';' { assignArrayVariable("EQL", $<object_ptr>9);}
;

IfStmt
    : IF '(' Condition ')' { ifBegin(); printf("IF\n"); } IfBlock ElseStmt
;
...
ElseStmt
    : ELSE { printf("ELSE\n"); elseBegin(); } Block  { elseEnd();}
    | { ifOnlyEnd(); }
;

WhileStmt
    : WHILE { printf("WHILE\n"); whileBegin();} '(' Condition ')' { whileBodyBegin();} Block { whileEnd();}
;

ForStmt
    : FOR { printf("FOR\n"); pushScope(); forBegin(); } '(' ForCondition ')' '{' StmtList '}'  { dumpScope(); forEnd();}
;

ForCondition
    : ForAssignStmt { forInitEnd();} ';' Condition ';' { forConditionEnd($4);} ForIncStmt { forHeaderEnd();}
    | ForRangeStmt { foreachHeaderEnd($1); }
;
...
ForIncStmt
    : IDENTITY INC_ASSIGN { objectIncAssignFor(findVariable($<s_var>1)); printf("INC_ASSIGN\n"); }
    | IDENTITY DEC_ASSIGN { objectDecAssignFor(findVariable($<s_var>1)); printf("DEC_ASSIGN\n"); }   
    | IDENTITY assign_op {printLoadFor(findVariable($<s_var>1));} Expression { assignVariableFor($<s_var>1, $<s_var>2, $<object_ptr>4); printf("%s_ASSIGN\n", $<s_var>2);}
;
...
ParamStmt
    ...
    | IDENT '[' Expression ']' '[' Expression ']' { create2DArray(OBJECT_TYPE_UNDEFINED, $<s_var>1, $3, $6); arrayEndStmt($<s_var>1);}
...
LitExpr 
    : IDENT '[' { arrayExprStmt($1);} ArrayExpr { $$ = $4;}
    | IDENT { 
...
ArrayExpr
    : Expression ']' { $$ = findArrayVariable("test", $1); arrayEndCout();}
    | Expression ']' { printExpr1($1); } '[' { codeRaw("aaload");} Expression ']' { $$ = find2DArrayVariable("test", $1, $6); arrayEndCout();}
;
