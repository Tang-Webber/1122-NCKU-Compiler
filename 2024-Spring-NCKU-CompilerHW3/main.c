...
bool inForStmt = false;
char* forStmt[5][10];
int forLayer = 0;
int forStmtNum[5] = {0,0,0,0,0};

int loopLabel = 0;
int ifElseLabel = 0;
int ifEndLabel = 0;
bool ifElseComplete[15] = {false,false,false,false,false,false,false,false,false,false,false,false,false,false,false} ;
bool ifEndComplete[15] ={false,false,false,false,false,false,false,false,false,false,false,false,false,false,false} ;
bool loopComplete[15] ={false,false,false,false,false,false,false,false,false,false,false,false,false,false,false} ;
...
void arrayExprStmt(char* name){
    arrayInitStmt = true;
    arrayName = name;
    Object* array = findVariable(name);
    code("aload %d", (int)array->symbol->addr);
}
void arrayEndStmt(char* name){
...
void functionEnd(ObjectType returnType){
    ...
    if(!hasReturn){
        ...
        /*
            ...
            if(strcmp(objectTypeName[returnType], "int") == 0 || strcmp(objectTypeName[returnType], "bool") == 0){
                codeRaw("ireturn"); 
            }
            if(strcmp(objectTypeName[returnType], "float") == 0){
                codeRaw("freturn"); 
            }
        }
        */
...
void assignVariableFor(char* varName, char* op, Object* expr){
    ...
    if (target->type == OBJECT_TYPE_INT) {
        if(expr->type == OBJECT_TYPE_FLOAT){
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("f2i");
        }
        if(strcmp("EQL", op) == 0){         // =
        }else if(strcmp("ADD", op) == 0){    // +=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("iadd");
        }else if(strcmp("SUB", op) == 0){    // -=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("isub");
        }else if(strcmp("MUL", op) == 0){    // *=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("imul");
        }else if(strcmp("DIV", op) == 0){    // /=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("idiv");
        }else if(strcmp("REM", op) == 0){    // %=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("irem");
        }else if(strcmp("BOR", op) == 0){    // |=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("ior");
        }else if(strcmp("BAN", op) == 0){    // &=
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("iand");
        }else if (strcmp("SHR", op) == 0){ 
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("ishr");
        }else if (strcmp("SHL", op) == 0){
            forStmt[forLayer][forStmtNum[forLayer]++] = strdup("ishl");
        }
        char buf[128] = {0};
        sprintf(buf, "istore %d", (int)target->symbol->addr);
        forStmt[forLayer][forStmtNum[forLayer]++] = strdup(buf);
    }
}
...
Object* createObject(char* variableType, char* str){
    ...
    if(strcmp(variableType, "i") == 0){
        ...
        if(arrayInitStmt){
            ...
            if(inForStmt){
                char buf[128] = {0};
                sprintf(buf, "ldc %d", (int)newObject->value.int_value);
                forStmt[forLayer][forStmtNum[forLayer]++] = strdup(buf);
            }
            else{
...
void ifBegin(){
    inIfStmt = true;
    code("ifeq ifElse%d", ifElseLabel);
    ifElseComplete[ifElseLabel++] = true;
}
void ifOnlyEnd(){
    inIfStmt = false;
    int i = ifElseLabel;
    while(!ifElseComplete[i]){
        i--;
    }
    code("ifElse%d:", i);
    ifElseComplete[i] = false;
}
void elseBegin(){
    inIfStmt = true;
    code("goto ifEnd%d", ifEndLabel);
    ifEndComplete[ifEndLabel++] = true;
    int i = ifElseLabel;
    while(!ifElseComplete[i]){
        i--;
    }
...
void elseEnd(){
    inIfStmt = false;
    int i = ifEndLabel - 1;
    while(!ifEndComplete[i]){
        i--;
    }
    ifEndComplete[i] = false;
    code("ifEnd%d:", i);
}
void whileBegin(){
    code("Loop%dStart:", loopLabel);
}
void whileBodyBegin(){
    code("ifeq Loop%dEnd", loopLabel);
    loopComplete[loopLabel++] = true;    
}
void whileEnd(){
    int i = loopLabel;
    while(!loopComplete[i]){
        i--;
    }
    loopComplete[i] = false;
    code("goto Loop%dStart", i);
    code("Loop%dEnd:", i);
}

void breakLoop(){
    code("goto Loop%dEnd", loopLabel - 1);
}
...
void forEnd(){
    for(int i = 0; i < forStmtNum[forLayer]; i++){
        code("%s", forStmt[forLayer][i]);
    }
    forStmtNum[forLayer] = 0;
    forLayer--;
    int i = loopLabel;
    while(!loopComplete[i]){
        i--;
...
void printLoadFor(Object* temp){
    char buf[128] = {0};
    if(strcmp(objectTypeName[temp->type], "int") == 0){
        sprintf(buf, "iload %d", (int)temp->symbol->addr);
    }
    if(strcmp(objectTypeName[temp->type], "float") == 0){
         sprintf(buf, "fload %d", (int)temp->symbol->addr);
    }  
    forStmt[forLayer][forStmtNum[forLayer]++] =  strdup(buf);  
}
...
Object* findArrayVariable(char* str, Object* expr){
    ...
    if(strcmp(expr->symbol->name, "int") == 0){
        code("ldc %d", arrayTemp->value.int_value);
    }
    codeRaw("iaload");
    return arrayObject;
...
Object* find2DArrayVariable(char* str, Object* expr1, Object* expr2){
    ...
    //codeRaw("aaload");
    if(strcmp(expr2->symbol->name, "int") == 0){
        
        code("ldc %d", expr2->value.int_value);
    }
    else{
        //code("iload %d", (int)expr2->symbol->addr);
    }    
    codeRaw("iaload");
...
void printExpr1(Object* expr){
    if(strcmp(expr->symbol->name, "int") == 0){
        code("ldc %d", expr->value.int_value);
    }    
}
...
Object* objectExpBoolean(char* op, Object* a, Object* b) {
    ...
    if (strcmp(op, "LOR") == 0) {
        ...
        if (a->type == OBJECT_TYPE_INT && b->type == OBJECT_TYPE_INT) {
            ...
            code("if_icmplt lessThanLabel%d", label);
            codeRaw("iconst_0");
            code("goto endLabel%d", label);
            code("lessThanLabel%d:", label);
            codeRaw("iconst_1");
            code("endLabel%d:", label);
            label++;         
    } 
    else if (strcmp(op, "GEQ") == 0) {      // Greater than or equal to
        code("if_icmpge greaterThanOrEqualToLabel%d", label);
        codeRaw("iconst_0");
        code("goto endLabel%d", label);
        code("greaterThanOrEqualToLabel%d:", label);
        codeRaw("iconst_1");
        code("endLabel%d:", label);
        label++;            
    } 
    else if (strcmp(op, "LEQ") == 0) {      // Less than or equal to
        code("if_icmple lessThanOrEqualToLabel%d", label);
        codeRaw("iconst_0");
        code("goto endLabel%d", label);
        code("lessThanOrEqualToLabel%d:", label);
        codeRaw("iconst_1");
        code("endLabel%d:", label);
        label++;           
    } 
    ...
    result->type = temp;
    result->symbol->name = strdup(objectTypeName[temp]);
...
Object* objectIncAssignFor(Object* a) {
    a->value.int_value++;
    char buf[128] = {0};
    sprintf(buf, "iinc %d 1", (int)a->symbol->addr);
    forStmt[forLayer][forStmtNum[forLayer]++] = strdup(buf);
...
Object* callFunction(char* funcName){
    ...
    code("invokestatic Main/%s%s", func->symbol->name, func->symbol->func_sig);
...
bool returnObject(Object* obj){
    ...
    if(strcmp(functionName, "main" ) == 0){
        ...
        if(obj != NULL){
            if(strcmp(objectTypeName[obj->type], "int") == 0 || strcmp(objectTypeName[obj->type], "bool") == 0){
                codeRaw("ireturn");
            }
            else if(strcmp(objectTypeName[obj->type], "float") == 0){
                codeRaw("freturn"); 
            }                
        }
        else {
