...
/* Function */
FunctionDefStmt
    : VARIABLE_T IDENT '(' { createFunction($1, $<s_var>2); } FunctionParameterStmtList ')' { funcSigAssign(); } '{' StmtList '}' { dumpScope(); }
;
...
SimpleStmt
    ...
    | IDENT '[' Expression ']' { printIdent(findVariable($<s_var>1)); loadArrayVariable($<s_var>1,  $<object_ptr>3); } VAL_ASSIGN Expression ';' { assignArrayVariable("EQL", $<object_ptr>2); printf("EQL_ASSIGN\n");}
    | IDENT '[' Expression ']' '[' Expression ']' { printIdent(findVariable($<s_var>1)); load2DArrayVariable($<s_var>1,  $<object_ptr>3, $<object_ptr>6); } VAL_ASSIGN Expression ';' { assignArrayVariable("EQL", $<object_ptr>2); printf("EQL_ASSIGN\n");}
...
LitExpr 
    ...
    | IDENT '[' Expression ']' '[' Expression ']' { $$ = find2DArrayVariable($1, $3, $6); printIdent($$);}
